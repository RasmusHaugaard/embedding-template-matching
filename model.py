...
class Model(pl.LightningModule):
    def __init__(self, rgba_template: np.ndarray, emb_dim=3, normalize=False, angle_res=16):
        super().__init__()
    ...
    def forward(self, img):
        ...
        # TODO: make sure that conv2d doesnt flip weights, or input
        if self.pad is not None:
            emb = F.pad(emb, (self.k, self.k, self.k, self.k), mode=self.pad)
